...
string Twitter::getElement(xmlDocPtr doc, xmlNodePtr cur, const char *keyword)
{
	xmlChar *key = NULL;
...
bool Twitter::switchTimeLine(int xmlSwitch){
	...
	case 1:
		this->timeLine.getTimelineFile().assign(config.getTimeLineDir());
		this->timeLine.getTimelineFile().append("home_timeline.xml");
		this->timeLine.getTimelineURL().assign(HOME_TIMELINE_URL);


	case 2:
		this->timeLine.getTimelineFile().assign(config.getTimeLineDir());
		this->timeLine.getTimelineFile().append("public_timeline.xml");
		this->timeLine.getTimelineURL().assign(PUBLIC_TIMELINE_URL);


	case 3:
		this->timeLine.getTimelineFile().assign(config.getTimeLineDir());
		this->timeLine.getTimelineFile().append("mentions.xml");
		this->timeLine.getTimelineURL().assign(MENTIONS_TIMELINE_URL);


	case 4:
		this->timeLine.getTimelineFile().assign(config.getTimeLineDir());
		this->timeLine.getTimelineFile().append("friends_timeline.xml");
		this->timeLine.getTimelineURL().assign(FRIENDS_TIMELINE_URL);


	case 5:
		this->timeLine.getTimelineFile().assign(config.getTimeLineDir());
		this->timeLine.getTimelineFile().append("user_timeline.xml");
		this->timeLine.getTimelineURL().assign(USER_TIMELINE_URL);


	case 6:
		this->timeLine.getTimelineFile().assign(config.getTimeLineDir());
		this->timeLine.getTimelineFile().append("retweeted_by_me.xml");
		this->timeLine.getTimelineURL().assign(RT_BY_ME_TIMELINE_URL);


	case 7:
		this->timeLine.getTimelineFile().assign(config.getTimeLineDir());
		this->timeLine.getTimelineFile().append("retweeted_to_me.xml");
		this->timeLine.getTimelineURL().assign(RT_TO_ME_TIMELINE_URL);


	case 8:
		this->timeLine.getTimelineFile().assign(config.getTimeLineDir());
		this->timeLine.getTimelineFile().append("retweeted_of_me.xml");
		this->timeLine.getTimelineURL().assign(RT_OF_ME_TIMELINE_URL);
...
bool Twitter::tokenAccess(const string pin){
	...
	if (twitterUserKey.compare(" ") ||twitterUserKey.empty() ) return false;
