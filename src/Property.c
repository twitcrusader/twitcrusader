...
void startWindowProperties(user_t *user, GtkWidget *window)
{
	...
	GError *error=NULL;
	gtk_window_set_icon_from_file (GTK_WINDOW(dialog), ICONS_DIR""ICON_SETTINGS, &error);
	if(error)
	{
	...
	gtk_table_attach (GTK_TABLE (table), label, 1, 11, 0, 1, GTK_FILL | GTK_EXPAND, GTK_FILL | GTK_EXPAND, 0, 0);


	if(user)
	{
		gtk_combo_box_text_append_text(GTK_COMBO_BOX_TEXT(combo), user->screenName);
		GtkWidget *button = gtk_button_new_with_label ("Delete");
		gtk_table_attach (GTK_TABLE (table), button, 4, 8, 5, 6, GTK_FILL | GTK_EXPAND, GTK_FILL | GTK_EXPAND, 0, 0);
		g_signal_connect (G_OBJECT (button), "clicked", G_CALLBACK (deleteAccount), NULL);

	}

	gtk_table_attach (GTK_TABLE (table), combo, 2, 10, 1, 2, GTK_FILL | GTK_EXPAND, GTK_FILL | GTK_EXPAND, 0, 0);
