...
extern	void	init_charbar		(string_t);
extern	void	startMainWindow		(	);
extern	void	refreshWindow		(	);
extern	void	updateTimeline		(	);
extern	void	homeTimeline		(	);
extern	void	mentionsTimeline	(	);
extern	void	loadWindowProperty	(	);
...
void updateStatusBar(GtkTextBuffer *buffer, GtkStatusbar *charbar)
{
	GtkTextIter iter;
 ...
 *
 */
gboolean gtkSendTweet(GtkWidget *TextArea, GdkEventKey *pKey, GtkTextBuffer *buffer)
{
	...
	if(msg)
	{
		...
		if(pKey->keyval == GDK_KEY_Return)
		{
			...
			gtk_statusbar_push (GTK_STATUSBAR(statusbar), 0, "Tweet was not sent..");
			gtk_text_buffer_delete(buffer, &start, &end);
...
void init_main_window()
{
	if(!window)
	{
		...
		GError *error=NULL;
		gtk_window_set_icon_from_file(GTK_WINDOW(window), ICONS_DIR""ICON_FAVICON, &error);
...
void init_toolbar()
{
	...
	if(!toolbar)
	{
		...
		const string_t titles[]={
				"Update", "Home", "Mentions", "Direct Messages", "Favorites", "Link", "Photo"
		};

		const string_t icons []=
		{
				ICONS_DIR""ICON_UPDATE,
				ICONS_DIR""ICON_HOME,
				ICONS_DIR""ICON_MENTION,
				ICONS_DIR""ICON_DM,
				ICONS_DIR""ICON_FAVORITES,
				ICONS_DIR""ICON_LINK,
				ICONS_DIR""ICON_PHOTO
		};

		const voidPtr_t functions []={updateTimeline, homeTimeline, mentionsTimeline,foo,foo,foo,foo};
...
void init_statusbar(string_t msg)
{
	if(!statusbar)
	{
...
void init_charbar(string_t msg)
{
	if(!charBar)
	{
...
void init_text_area()
{
	if(!scroll)
	{
		...
		g_signal_connect(buffer, "changed", G_CALLBACK(updateStatusBar), charBar);
...
void init_scrolled_window()
{
	...
	if(!tableTW)
	{
		...
		gtk_scrolled_window_add_with_viewport (GTK_SCROLLED_WINDOW (scrolled_window), tableTW);
	}
...
void init_timeline(timeline_t timeline)
{
	...
	for (cols=0; cols < MAX_NUM_TWEETS; rows = rows + 4, cols++)
	{
		if(timeline.timeline[cols].user.profile_image || timeline.timeline[cols].user.screen_name || timeline.timeline[cols].text || timeline.timeline[cols].created_at)
		{
			...
			gtk_table_attach (GTK_TABLE (tableTW ), gtweet, 1, 10, rows, rows + 3, GTK_FILL | GTK_SHRINK, GTK_FILL | GTK_SHRINK, 0, 0);
		}
		else
			rows=rows-4;
...
void startMainWindow()
{
	if(!window)
	{
		...
		gtk_table_attach (GTK_TABLE (table), scrolled_window, 0, 3, 0, 7, GTK_FILL| GTK_EXPAND, GTK_FILL | GTK_EXPAND, 0, 0);
		gtk_table_attach (GTK_TABLE (table), toolbar, 0, 3, 7, 8, GTK_FILL| GTK_EXPAND, GTK_FILL | GTK_EXPAND, 0, 0);
...
static void leftClick(GtkStatusIcon *icon, gpointer data)
{
...
static void rightClick(GtkStatusIcon *icon, guint button, guint activate_time, gpointer data)
{

	GtkWidget *menu;


	const string_t titles[]= {"Update", "Settings", "Version", "About","Quit"};
	const string_t icons []={ICONS_DIR""ICON_UPDATE_TRAYICON, ICONS_DIR""ICON_SETTINGS, ICONS_DIR""ICON_UPGRADE, ICONS_DIR""ICON_STAR, ICONS_DIR""ICON_CLOSE};
	const voidPtr_t functions[]= {updateTimeline, loadWindowProperty, startVersionWindow, startCreditsDialog, onQuit};
	...
	for(i=0; i<5; i++)
	{
		...
		g_signal_connect(G_OBJECT(item), "activate",G_CALLBACK(functions[i]), NULL);
...
void startTrayIcon()
{
	...
	g_signal_connect(G_OBJECT(icon),"popup-menu",G_CALLBACK(rightClick), NULL);
