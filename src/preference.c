...
extern "C"
{
  ...
  string_t MakeAvatarName(string_t url, string_t screen_name, string_t dir)
  {
  ...
  byte_t _downloadAvatar(string_t url, string_t screen_name, string_t dir)
  {
  ...
  void downloadAvatar(progData_t *progData)
  {
    ...
    for (i = 0; i < MAX_NUM_TWEETS; i++)
      {
        ...
        if(progData->home_tl.statuses[i].user.profile_image_url && progData->home_tl.statuses[i].user.screen_name)
          _downloadAvatar(progData->home_tl.statuses[i].user.profile_image_url, progData->home_tl.statuses[i].user.screen_name, progData->pp->avatarDir);

        if(progData->mentions_tl.statuses[i].user.profile_image_url && progData->mentions_tl.statuses[i].user.screen_name)
          _downloadAvatar(progData->mentions_tl.statuses[i].user.profile_image_url, progData->mentions_tl.statuses[i].user.screen_name, progData->pp->avatarDir);

        if(progData->favorites_tl.statuses[i].user.profile_image_url && progData->favorites_tl.statuses[i].user.screen_name)
          _downloadAvatar(progData->favorites_tl.statuses[i].user.profile_image_url, progData->favorites_tl.statuses[i].user.screen_name, progData->pp->avatarDir);
    ...
    for (i = 0; i < MAX_NUM_DM; i++)
      {
        ...
        if(progData->dm_rx.directMessage[i].sender.profile_image_url && progData->dm_rx.directMessage[i].sender.screen_name, progData->pp->avatarDir)
          _downloadAvatar(progData->dm_rx.directMessage[i].sender.profile_image_url, progData->dm_rx.directMessage[i].sender.screen_name, progData->pp->avatarDir);

        if(progData->dm_tx.directMessage[i].sender.profile_image_url && progData->dm_tx.directMessage[i].sender.screen_name)
          _downloadAvatar(progData->dm_tx.directMessage[i].sender.profile_image_url, progData->dm_tx.directMessage[i].sender.screen_name, progData->pp->avatarDir);
      }
  }


  void initProgData(progData_t *progData)
  {

    progData->pp=initProgPath(PROG_PATH, AVATAR_DIR, CONFIG_DIR,
  ...
  void updateProgData(progData_t *progData)
  {
    ...
    if(progData->user)
      {
        ...
        string_t raw = getRawTimeline(progData->twURLS, home_timeline, progData->user);
  ...
  void uninitProgData(progData_t *progData)
  {
