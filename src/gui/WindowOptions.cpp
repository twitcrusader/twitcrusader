...
gboolean WindowOptions::declaration()
{
	...
	if(!twitterStruct.twitter.getLocalUser().getScreenName().empty()){
		itemsAccount = g_list_append (itemsAccount, (void*)twitterStruct.twitter.getLocalUser().getScreenName().c_str());
	}else{
	...
	if(twitterStruct.twitter.getLocalUser().getId().empty() || twitterStruct.twitter.getLocalUser().getScreenName().empty()){
		...
		gtk_table_attach (GTK_TABLE (table), button, 3, 7, 5, 6, (GtkAttachOptions)(GTK_FILL | GTK_EXPAND),(GtkAttachOptions)(GTK_FILL | GTK_EXPAND), 0, 0);
		g_signal_connect (G_OBJECT (button), "clicked", G_CALLBACK (&WindowOptions::gtkDeleteAccount), NULL);
	}
...
gboolean WindowOptions::UpdateWindow()
{
	...
	//gtk_widget_draw(GTK_WIDGET(window), NULL); Not Work!!
	return true;
}

gboolean WindowOptions::ShowWindow()
{
	if(twitterStruct.twitter.getLocalUser().getScreenName().empty()|| twitterStruct.twitter.getLocalUser().getId().empty()){
...
gboolean WindowOptions::gtkDeleteAccount(){

	if(twitterStruct.twitter.getConfig().deleteConfigFile()){

		WindowOptions::UpdateWindow();
		return true;
	}
return false;
}
