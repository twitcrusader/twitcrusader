...
gboolean WindowAddUsers::declaration()
{
	...
	g_signal_connect (G_OBJECT (button), "clicked", G_CALLBACK(&WindowAddUsers::gtkAccessToken), pin);
...
{
	gtk_widget_draw(GTK_WIDGET(window), NULL);
	return true;
}
...
gboolean WindowAddUsers::gtkAccessToken(GtkWidget *pin){

	bool correctVerify;

	/* Get text from GTK_Entry*/
	const gchar *str=gtk_entry_get_text (GTK_ENTRY (pin));

	//Validate PIN
	correctVerify = twitterStruct.twitter.tokenAccess(str);
	...
	if(correctVerify){
		...
		windowError((gchar*)"bad Input!");
	}

	return false;
}

void WindowAddUsers::windowError(gchar* msg){

	/* Set all window options (color, size, position, etc) */

	GtkWidget *dialog=gtk_dialog_new ();
	GtkWidget *labelErr;
	GtkWidget *tableErr = gtk_table_new (5, 10, TRUE);



	gtk_window_set_default_size (GTK_WINDOW(dialog), 150, 50);
	...
	/* Error Message */
	labelErr = gtk_label_new (msg);
	gtk_table_attach (GTK_TABLE (tableErr), labelErr, 1, 9, 1, 3, (GtkAttachOptions)(GTK_FILL | GTK_EXPAND), (GtkAttachOptions)(GTK_FILL | GTK_EXPAND), 0, 0);

	/* Attach tabke at dialog container */
	gtk_container_add (GTK_CONTAINER ( gtk_dialog_get_content_area (GTK_DIALOG (dialog))), tableErr);
